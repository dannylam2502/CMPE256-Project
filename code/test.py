...
class DetailedObserverPlayer(Player):
    def choose_move(self, battle):
        with torch.no_grad():
            ...
            print(torch.softmax(model(dummy_batch), dim=1))
    ...
    def parse_pokemon(self, pokemon):
        moves = [{'name': name, 'current_pp': pokemon.moves[name].current_pp} for name in pokemon.moves]
...
async def main():
    player_1 = DetailedObserverPlayer(battle_format="gen3randombattle", max_concurrent_battles=1)
    player_2 = RandomPlayer(battle_format="gen3randombattle", max_concurrent_battles=1)
